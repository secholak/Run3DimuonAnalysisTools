...
dxyz_signif_cut = 3 #sigma, where dXYZ / errdXYZ > 3 is considered "displaced"
#  displacement dxyz = sqrt((vtxX - pvX)^2 + (vtxY - pvY)^2 + (vtxZ - pvZ)^2) / sqrt( errvtxX^2 + errvtxY^2 + errvtxZ^2 )
...
def fillHistograms() :
	...
	for ev in itree :
		...
		dXYZ = math.sqrt( (ev.vtxX - ev.pvX)**2 + (ev.vtxY - ev.pvY)**2 + (ev.vtxZ - ev.pvZ)**2 )
		errdXYZ = math.sqrt( ev.vtxXError**2 + ev.vtxYError**2 + ev.vtxZError**2 )
		if errdXYZ > 0 :	
			dxyz_signif = dXYZ / errdXYZ
		else :
			dxyz_signif = 0
		displ_key = 1 if dxyz_signif < dxyz_signif_cut else 2


		if abs(ev.eta1) < 1.48 and abs(ev.eta2) < 1.48 and ev.pt > pt_cut_bb :
			histo_name = f"Cat_{displ_key}_bb"
		elif abs(ev.eta1) > 1.48 and abs(ev.eta2) > 1.48 and abs(ev.eta1) < 3 and abs(ev.eta2) < 3 and ev.pt > pt_cut_ee :
			histo_name = f"Cat_{displ_key}_ee"
		else :
			continue
